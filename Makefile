...
uninit-full: clear uninit
	rm -rf .idea design-patterns.iml .git
...
build:
	./mvnw verify
	chmod +x ./target/design-patterns-0.0.1-SNAPSHOT.jar

run:
	./mvnw spring-boot:start -Dspring.profiles.active=local
#	./target/design-patterns-0.0.1-SNAPSHOT.jar
#	java -jar --enable-preview ./target/design-patterns-0.0.1-SNAPSHOT.jar
...
git-fork-init: init
	git remote add upstream git://github.com/Vyacheslav-Lapin/design-patterns.git
	git fetch upstream
